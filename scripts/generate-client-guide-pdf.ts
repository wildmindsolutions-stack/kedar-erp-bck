/**
 * Generates Kedar_ERP_Client_Role_Guide.pdf for client handover / UAT.
 * Run: cd backend && npx ts-node scripts/generate-client-guide-pdf.ts
 */
...
table(
  ...
  ROLES.map((r) => [r.role, r.name, r.email, DEMO_PASSWORD]),
  [80, 110, 200, 80],
);

h1('3. Module Access Summary');
table(
  ['Module', 'Owner', 'Manager', 'Accountant', 'Warehouse', 'Sales'],
  [
...
table(
  ['Screen', 'Main actions', 'Who can access'],
  screens.map((s) => [s.name, s.actions, s.roles]),
  [95, 250, 120],
);
...
doc.fontSize(8).fillColor(muted).text(
  'Kedar Enterprise ERP · Client Role Guide · Phase 1 · WildMind Solutions · Confidential',
  { align: 'center' },
);
